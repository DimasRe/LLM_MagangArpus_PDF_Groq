...
def extract_text_and_tables_from_pdf(file_path: str) -> str:
    """Mengekstrak teks dan mengonversi tabel dari file PDF menjadi format Markdown."""
    try:
        ...
        for page_num, page in enumerate(doc):
            ...
            # 2. Cari semua tabel di halaman secara aktif
            ...
            if tables:
                for i, table in enumerate(tables):
                    try:
                        ...
                        # 4. Konversi data tabel ke format string Markdown yang rapi
                        markdown_table = f"\n\n--- Tabel {i+1} dari Halaman {page_num + 1} ---\n"
                        ...
                        for row in table_data[1:]:
                            ...
                            markdown_table += f"| {' | '.join(clean_row)} |\n"
                        markdown_tables += markdown_table + "--- Akhir Tabel ---\n"
                    except Exception as e:
                        print(f"Gagal memproses tabel {i+1} di halaman {page_num+1}: {e}")
            
            # 5. Gabungkan teks biasa dengan tabel yang sudah diformat
            full_content += page_text + markdown_tables + "\n\n"
